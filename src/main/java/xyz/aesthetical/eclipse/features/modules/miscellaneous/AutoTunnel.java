...
public class AutoTunnel extends Module {
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (Module.fullNullCheck() && event.getEntityLiving() == Eclipse.mc.player) {
            if (!blocks.isEmpty()) {
                ...
                EnumFacing facing = Eclipse.mc.player.getHorizontalFacing();
    ...
    private boolean shouldBreak() {
        return Eclipse.mc.player.movementInput.moveForward == 0.0f && Eclipse.mc.player.movementInput.moveStrafe == 0.0f;
    }
}
