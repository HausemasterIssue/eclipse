...
public class KillAura extends Module {
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (Module.fullNullCheck() && event.getEntityLiving() == Eclipse.mc.player) {
            if (target == null || target.isDead || Eclipse.mc.player.getDistance(target) > range.getValue().floatValue()) {
            ...
            if (timeCheckPassed()) {
                ...
                if (sync.getValue()) {
                    Eclipse.mc.player.connection.sendPacket(new CPacketUseEntity(target));
                } else {
                    Eclipse.mc.playerController.attackEntity(Eclipse.mc.player, target);
                }
    ...
    private boolean timeCheckPassed() {
        switch (delay.getValue()) {
            ...
            case VANILLA:
                return Eclipse.mc.player.getCooledAttackStrength(0.0f) == 1.0f;
