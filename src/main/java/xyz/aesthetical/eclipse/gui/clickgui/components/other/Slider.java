package xyz.aesthetical.eclipse.gui.clickgui.components.other;
...
public class Slider extends Component {
    ...
    public void draw(int mouseX, int mouseY, float tickDelta) {
        ...
        RenderUtils.drawRect(x, y, setting.getValue().floatValue() <= setting.getMin().floatValue() ? 0.0f : width * partialMultiplier(), height, ColorUtil.toRGBA(237, 47, 47, 255));
        Eclipse.textManager.draw(setting.getName() + ": " + setting.getValue(), (float) (x) + 2.3f, toCenterHeight((float) y, (float) height), -1);
    }
