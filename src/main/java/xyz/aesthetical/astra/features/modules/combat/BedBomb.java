...
public class BedBomb extends Module {
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (Module.fullNullCheck() && event.getEntityLiving() == Astra.mc.player) {
            Biome biome = Astra.mc.world.getBiome(Astra.mc.player.getPosition());
    ...
    private void doBedBomb() {
        if (placePos == null || direction == null) {
            ...
            if (!(Astra.mc.world.getBlockState(placePos).getBlock() instanceof BlockBed)) {
                return;
            }
    ...
    private void findPlacePositions() {
        List<BlockPos> placePositions = WorldUtils.getSphere(new BlockPos(Astra.mc.player.getPositionVector()), placeRange.getValue().intValue(), placeRange.getValue().intValue(), false, true, 0);
        if (!placePositions.isEmpty()) {
            ...
            for (BlockPos pos : placePositions) {
                ...
                for (EnumFacing direction : EnumFacing.values()) {
                    ...
                    if (!Astra.mc.world.getBlockState(pos.offset(direction)).getMaterial().isReplaceable()) {
                        dir = direction;
                        break;
                    }
                }
                ...
                float selfDamage = DamageUtils.calculateDamage(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, 5.0f, true, Astra.mc.player);
                if (!suicide.getValue()) {
                    if (selfDamage - 0.5 >= maxSelfDamage.getValue().floatValue() || selfDamage - 0.5 >= EntityUtil.getTotalHealth(Astra.mc.player)) {
                        continue;
                    }
                }
    ...
    private void sleepInBed(EnumHand h) {
        if (Astra.mc.world.getBlockState(placePos).getBlock() instanceof BlockBed) {
            ...
            if (sync.getValue()) {
                ...
                Astra.mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(placePos, facing, h, x, y, z));
            } else {
                Astra.mc.playerController.processRightClickBlock(Astra.mc.player, Astra.mc.world, placePos, facing, hitVector, h);
            }
        }
    }
