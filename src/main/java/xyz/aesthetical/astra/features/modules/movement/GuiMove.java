...
public class GuiMove extends Module {
    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (Module.fullNullCheck() && event.getEntityLiving() == Astra.mc.player && isValidGUI()) {
            GameSettings settings = Astra.mc.gameSettings;
            ...
            if (arrowLook.getValue()) {
                ...
                if (Keyboard.isKeyDown(Keyboard.KEY_UP)) {
                    Astra.mc.player.rotationPitch = Math.max(Astra.mc.player.rotationPitch - offset, -90.0f);
                } else if (Keyboard.isKeyDown(Keyboard.KEY_DOWN)) {
                    Astra.mc.player.rotationPitch = Math.min(Astra.mc.player.rotationPitch + offset, 90.0f);
                } else if (Keyboard.isKeyDown(Keyboard.KEY_RIGHT)) {
                    Astra.mc.player.rotationYaw += offset;
    ...
    public boolean isValidGUI() {
        return Astra.mc.currentScreen != null && !(Astra.mc.currentScreen instanceof GuiChat);
    }
}
