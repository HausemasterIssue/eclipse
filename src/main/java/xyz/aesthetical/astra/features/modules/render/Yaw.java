    ...
    public void onUpdate(LivingEvent.LivingUpdateEvent event) {
        if (Module.fullNullCheck() && event.getEntityLiving() == Astra.mc.player) {
            if (yawDirection.getValue() == Direction.CUSTOM) {
                Astra.mc.player.rotationYaw = customYaw.getValue().floatValue();
            } else {
                Astra.mc.player.rotationYaw = yawDirection.getValue().getYaw();
            }

            if (lockPitch.getValue()) {
                Astra.mc.player.rotationPitch = customPitch.getValue().floatValue();
            }
        }
    }
