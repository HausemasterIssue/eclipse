...
public class Search extends Command {
    ...
    public void execute(Minecraft client, List<String> args) throws Exception {
        ...
        if (action.equalsIgnoreCase("add")) {
            if (!xyz.aesthetical.astra.features.modules.render.Search.blocks.contains(block)) {
                xyz.aesthetical.astra.features.modules.render.Search.blocks.add(block);
            } else {
                throw new InvalidArgumentException("block name", "Already is a search block");
            }
        } else if (action.equalsIgnoreCase("remove")) {
            if (xyz.aesthetical.astra.features.modules.render.Search.blocks.contains(block)) {
                xyz.aesthetical.astra.features.modules.render.Search.blocks.remove(block);
            } else {
