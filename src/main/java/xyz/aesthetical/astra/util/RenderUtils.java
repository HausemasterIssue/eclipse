...
import xyz.aesthetical.astra.mixin.mixins.render.entities.IEntityRenderer;
...
public class RenderUtils {
    ...
    public static void renderNametag(String text, Vec3d pos, double yOffset, float transparency) {
        ...
        GlStateManager.rotate(-Astra.mc.player.rotationYaw, 0f, 1f, 0f);
        GlStateManager.rotate(Astra.mc.player.rotationPitch, 1f, 0f, 0f);
        ...
        float i = Astra.textManager.getWidth(text) / 2f;
    ...
    public static void drawTracer(Entity entity, float width, boolean smooth, boolean stem, int color) {
        Vec3d eyes = Astra.mc.player.getLookVec();
        double x = interpolate(entity.posX, entity.lastTickPosX) - Astra.mc.getRenderManager().viewerPosX;
        double y = interpolate(entity.posY, entity.lastTickPosY) - Astra.mc.getRenderManager().viewerPosY;
        double z = interpolate(entity.posZ, entity.lastTickPosZ) - Astra.mc.getRenderManager().viewerPosZ;
        ...
        GL11.glLoadIdentity();
        ((IEntityRenderer) Astra.mc.entityRenderer).doOrientCamera(Astra.mc.getRenderPartialTicks());

        bufferbuilder.begin(GL11.GL_LINES, DefaultVertexFormats.POSITION_COLOR);
        bufferbuilder.pos(eyes.x, eyes.y + Astra.mc.player.getEyeHeight(), eyes.z).color(red, green, blue, alpha).endVertex();
        bufferbuilder.pos(x, y, z).color(red, green, blue, alpha).endVertex();
    ...
    public static Vec3d getCameraPos() {
        RenderManager manager = Astra.mc.getRenderManager();
    ...
    public static double interpolate(double start, double end) {
        return end + (start - end) * Astra.mc.getRenderPartialTicks();
    }
