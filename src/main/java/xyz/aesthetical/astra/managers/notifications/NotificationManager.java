package xyz.aesthetical.astra.managers.notifications;
...
import xyz.aesthetical.astra.features.modules.client.Notifications;
import xyz.aesthetical.astra.features.sounds.NotificationSound;
...
public class NotificationManager {
        ...
        if (Module.fullNullCheck() && !notifications.isEmpty() && Notifications.instance.mode.getValue() == Notifications.Mode.ONSCREEN) {
            ...
            for (int i = 0; i < notifications.size(); ++i) {
                ...
                for (String line : notification.getLines()) {
                    Astra.textManager.draw(line, x + 2.3f, textY, -1);
                    textY += Astra.textManager.getHeight() + 2.0f;
                }
    ...
    private float getHeight(List<String> lines) {
        return lines.size() * (Astra.textManager.getHeight() + 3.0f);
    }

    private int getWidth(List<String> lines) {
        return (int) lines.stream().map(l -> Astra.textManager.getWidth(l)).sorted().toArray()[0];
    }

    public void createNotification(Notification notification) {
        if (Notifications.instance.mode.getValue() == Notifications.Mode.CHAT) {
            Astra.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion(
                    new TextComponentString(Astra.commandManager.getSentCommandPrefix() + String.join("\n", notification.getLines())),
    ...
    public void playNotificationSound() {
        switch (Notifications.instance.sound.getValue()) {
            case ANVIL:
                Astra.mc.getSoundHandler().playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.BLOCK_ANVIL_LAND, 1.0f));
                break;

            case ECLIPSE:
                Astra.mc.getSoundHandler().playSound(NOTIFICATION_SOUND);
                break;
