...
import xyz.aesthetical.astra.features.commands.*;
...
public class CommandManager {
    ...
    public void onInput(InputEvent.KeyInputEvent event) {
        ...
        if (!Keyboard.getEventKeyState() && key != Keyboard.KEY_NONE) {
            if (Astra.commandManager.getPrefix().equalsIgnoreCase(Keyboard.getKeyName(key))) {
                Astra.mc.displayGuiScreen(new GuiChat(Astra.commandManager.getPrefix()));
            }
        }
    }
    ...
    public void execute(String message) {
        ...
        if (!args.isEmpty()) {
            ...
            if (command != null) {
                try {
                    command.execute(Astra.mc, args.subList(1, args.size()));
                } catch (Exception e) {
    ...
    public String getSentCommandPrefix() {
        return new TextBuilder()
                .append(ChatColor.Blue, Astra.MOD_NAME)
                .append(" ")
                .append(ChatColor.Dark_Gray, "\u00BB")
                .append(" ")
                .build();
